...
static const havk::ShaderPrepDefs DefaultShaderDefs = {
...
struct GpuVoxelStorage {
    ...
    GpuVoxelStorage(havk::DeviceContext* ctx) {
        Device = ctx;
        BuildOccupancyShader = ctx->PipeBuilder->CreateCompute("UpdateOccupancy.slang", DefaultShaderDefs);
    }
    ...
    void SyncBuffers(VoxelMap& map) {
        ...
        auto mappedStorage = (GpuMeta*)StorageBuffer->MappedData; // write only!
        ...
        if (updateBatch.empty()) {
            StorageBuffer->Flush(offsetof(GpuMeta, Palette), sizeof(GpuMeta::Palette));
            return;
        }

        // Upload brick data
        auto updateBuffer = Device->CreateBuffer({
            .Size = dirtyBricksInBatch * sizeof(glm::uvec3) + sizeof(UpdateRequest),
            .Usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
            .VmaFlags = VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT,
        });
        auto updateLocs = (UpdateRequest*)updateBuffer->MappedData;
