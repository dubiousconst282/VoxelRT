...
struct Camera {
    ...
    glm::mat4 GetViewMatrix(bool translateToView = true) const {
    ...
    glm::mat4 GetProjMatrix() const {
        ...
        float f = 1.0f / tan(glm::radians(FieldOfView) / 2.0f);
        ...
        if (!ImGui::IsWindowFocused(ImGuiFocusedFlags_AnyWindow)) {
            ...
            if (Mode == InputMode::FirstPerson) {
                ...
                if (!ImGui::IsWindowHovered(ImGuiHoveredFlags_AnyWindow)) {
                    ArcDistance = glm::max(ArcDistance + io.MouseWheel * 0.5f, NearZ);
                }
