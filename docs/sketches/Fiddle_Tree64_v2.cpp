...
void Traversal(float3 origin, float3 dir) {
   ...
   // pos = clamp(pos, 1.0f, 1.9999999f);
    ...
    for (int j = 0; j < 16; j++) {
        ...
        int adv_scale = scaleExp;

        // TODO: faster anisotropic skips possible?
        // 2³ skips
        if ((node->PopMask >> (childIdx & 0b101010) & 0x00330033) == 0) {
            adv_scale++;
        }

        // Compute next hit pos
        float scale = std::bit_cast<float>((adv_scale - 23 + 127) << 23);  // 2 ^ -(23 - adv_scale)
        ...
        tmin = min(min(sideDist.x, sideDist.y), sideDist.z)+0.0001;
